...
async def main():
    ...
    enqueue("process_task", {"task_id": 1, "priority": 1}, queue="tasks", priority=1)
    enqueue(
        "process_task",
    ...
    enqueue("process_task", {"task_id": 3, "priority": 5}, queue="tasks", priority=5)
    enqueue("process_task", {"task_id": 4, "priority": 50}, queue="tasks", priority=50)
    enqueue("process_task", {"task_id": 5, "priority": 10}, queue="tasks", priority=10)

    print("Tasks enqueued in order: 1, 2, 3, 4, 5")
    print("Expected execution order by priority: 2(100), 4(50), 5(10), 3(5), 1(1)\n")
